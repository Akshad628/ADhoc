...
import { FileText, Upload, Trash2, Eye, History, Filter, ExternalLink } from 'lucide-react'
...
export default function DocumentsTab() {
  ...
  const handleUpload = async (file: File) => {
    const cat = CATEGORIES.find(c => c.value === uploadCategory)
  ...
  const handleDelete = async (doc: StudentDocument) => {
    if (!confirm(`Delete "${doc.original_file_name}"? This cannot be undone.`)) return
            ...
            <div className="grid grid-cols-2 gap-3 mb-4">
              <div>
                ...
                <select value={uploadCategory}
                  onChange={e => { setUploadCategory(e.target.value as DocumentCategory); setUploadSubCategory('') }}
                  className="w-full bg-white/[0.03] border border-white/10 rounded-xl px-3 py-2.5 text-white text-sm focus:outline-none focus:border-purple-500/50">
        ...
        {replaceDoc && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }}
            ...
            <p className="text-amber-400 text-sm font-medium mb-3">
              Replacing: {replaceDoc.original_file_name} (current v{replaceDoc.version_number})
            </p>
              ...
              <div className="flex items-start justify-between gap-3">
                <div className="flex items-start gap-3 flex-1 min-w-0">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-white text-sm font-medium truncate">{doc.original_file_name}</p>
                    <div className="flex items-center gap-2 flex-wrap mt-1">
                      <span className="text-zinc-600 text-xs capitalize">{doc.category}</span>
                      {doc.sub_category && <span className="text-zinc-700 text-xs">• {doc.sub_category.replace(/_/g, ' ')}</span>}
                      {doc.file_size_bytes && <span className="text-zinc-700 text-xs">• {formatBytes(doc.file_size_bytes)}</span>}
                      {doc.version_number > 1 && (
                        <span className="text-zinc-600 text-xs">• v{doc.version_number}</span>
                      )}
                    </div>
                    <div className="mt-2">
                      <VerificationBadge status={doc.verification_status} reviewComments={doc.review_comments}
                        rejectionReason={doc.rejection_reason} verifiedAt={doc.verified_at} />
                    </div>
                  </div>
                </div>
              ...
              {hasOCR(doc) && (
                <div className="mt-3 pt-3 border-t border-white/5">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                    {Object.entries((doc.ocr_metadata as Record<string, unknown>)?.extracted || {}).slice(0, 4).map(([field, val]: [string, unknown]) => {
